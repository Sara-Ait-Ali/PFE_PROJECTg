...
//         router.push(`/download-status?job_id=${res.data.job_id}&type=download`);
//       } else {
//         router.push(`/status?job_id=${res.data.job_id}&type=tmy`);
//       }
//     } catch (err) {
//       setError('Failed to submit job. Please try again.');
//     } finally {
...
//             >
//               <h2 className="text-xl font-bold mb-4">Select Files To Download</h2>
//               {['ERA5', 'CAMS', 'Temperature', 'GHI', 'DNI', 'GTI', 'Wind Speed', 'Relative Humidity'].map(item => (
...
//                       } else {
//                         setSelectedFiles(prev => prev.filter(x => x !== item));
//                       }
//                     }}
//                   />
//                   {item}
...
//                 onClick={() => {
//                   setForm(prev => ({ ...prev, job_type: 'download_only' }));
...
export default function GeneratePage() {
  ...
  const isFormValid = form.latitude && form.longitude && form.start_date && form.end_date && yearDiff >= 9;
  ...
  return (
    <>
      <style>{`
        .react-datepicker__header { background-color: #8DC63F !important; border-bottom: none !important; }
        .react-datepicker__current-month, .react-datepicker__day-name, .react-datepicker-year-header { color: white !important; font-weight: 600 !important; }
        .react-datepicker__day--selected, .react-datepicker__day--keyboard-selected { background-color: #8DC63F !important; color: white !important; border-radius: 6px !important; }
        .react-datepicker__day:hover { background-color: #EAF5CE !important; color: #4E7A1A !important; border-radius: 6px !important; }
        .react-datepicker__navigation-icon::before { border-color: white !important; }
        .react-datepicker__year-dropdown, .react-datepicker__month-dropdown { background: white !important; border: 1px solid #C8E47A !important; border-radius: 8px !important; }
        .react-datepicker__year-option:hover, .react-datepicker__month-option:hover { background-color: #EAF5CE !important; color: #4E7A1A !important; }
        .react-datepicker-wrapper, .react-datepicker__input-container { width: 100% !important; }
      `}</style>

      <div style={{ minHeight: '100vh', background: '#F4FAE8' }}>
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 24px 0' }}>
          <span style={{ background: '#EAF5CE', color: '#6A9E28', fontSize: '12px', fontWeight: 600, padding: '3px 10px', borderRadius: '999px', letterSpacing: '0.05em', textTransform: 'uppercase' }}>New Job</span>
          <h1 style={{ fontSize: '28px', fontWeight: 800, color: '#4E7A1A', margin: '8px 0 6px' }}>Generate TMY File</h1>
          <p style={{ color: '#6b7280', fontSize: '15px', margin: 0 }}>Click on the map to pick a location — coordinates and site name fill automatically.</p>
        </div>
        ...
        <div style={{ maxWidth: '1100px', margin: '24px auto', padding: '0 24px 40px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
          ...
          <div style={{ background: 'white', borderRadius: '20px', border: '1px solid #E5E7EB', boxShadow: '0 10px 30px rgba(141,198,63,0.08)', overflow: 'hidden' }}>
            ...
            <div style={{ height: '420px' }}>
              <MapPicker lat={parseFloat(form.latitude) || null} lon={parseFloat(form.longitude) || null} onLocationSelect={handleMapClick} />
            </div>
            <div style={{ padding: '10px 20px', background: '#F4FAE8', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <span style={{ fontSize: '12px', color: '#6b7280' }}>Click anywhere on the map to set coordinates</span>
            </div>
          ...
          <div style={{ background: 'white', borderRadius: '20px', border: '1px solid #E5E7EB', boxShadow: '0 10px 30px rgba(141,198,63,0.08)', padding: '24px' }}>
            ...
            {error && (
              <div style={{ background: '#fef2f2', border: '1px solid #fecaca', color: '#dc2626', padding: '10px 14px', borderRadius: '10px', fontSize: '13px', marginBottom: '16px' }}>{error}</div>
            )}

            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#374151', marginBottom: '6px' }}>Site Name</label>
                <input type="text" required value={form.site_name} onChange={(e) => setForm({ ...form, site_name: e.target.value })} placeholder="e.g. Marrakech-Morocco" style={inputStyle} onFocus={e => e.target.style.border = '1.5px solid #8DC63F'} onBlur={e => e.target.style.border = '1.5px solid #D1D5DB'} />
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#374151', marginBottom: '6px' }}>Latitude</label>
                  <input type="number" step="any" required value={form.latitude} onChange={(e) => setForm({ ...form, latitude: e.target.value })} placeholder="31.6295" style={inputStyle} onFocus={e => e.target.style.border = '1.5px solid #8DC63F'} onBlur={e => e.target.style.border = '1.5px solid #D1D5DB'} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#374151', marginBottom: '6px' }}>Longitude</label>
                  <input type="number" step="any" required value={form.longitude} onChange={(e) => setForm({ ...form, longitude: e.target.value })} placeholder="-7.9811" style={inputStyle} onFocus={e => e.target.style.border = '1.5px solid #8DC63F'} onBlur={e => e.target.style.border = '1.5px solid #D1D5DB'} />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#374151', marginBottom: '6px' }}>Start Date</label>
                  <DatePicker selected={form.start_date} onChange={(date) => setForm({ ...form, start_date: date })} dateFormat="dd/MM/yyyy" showMonthDropdown showYearDropdown dropdownMode="select" customInput={<input style={{ ...inputStyle, cursor: 'pointer' }} onFocus={e => e.target.style.border = '1.5px solid #8DC63F'} onBlur={e => e.target.style.border = '1.5px solid #D1D5DB'} />} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#374151', marginBottom: '6px' }}>End Date</label>
                  <DatePicker selected={form.end_date} onChange={(date) => setForm({ ...form, end_date: date })} dateFormat="dd/MM/yyyy" showMonthDropdown showYearDropdown dropdownMode="select" minDate={form.start_date ? new Date(new Date(form.start_date).setFullYear(form.start_date.getFullYear() + 9)) : null} customInput={<input style={{ ...inputStyle, cursor: 'pointer' }} onFocus={e => e.target.style.border = '1.5px solid #8DC63F'} onBlur={e => e.target.style.border = '1.5px solid #D1D5DB'} />} />
                </div>
              </div>

              {form.start_date && form.end_date && (
                yearDiff < 9 ? (
                  <div style={{ background: '#fef9c3', border: '1px solid #fde047', color: '#854d0e', padding: '10px 14px', borderRadius: '10px', fontSize: '13px' }}>⚠️ Minimum period is 9 years. Currently: {Math.floor(yearDiff)} years.</div>
                ) : (
                  <div style={{ background: '#F4FAE8', border: '1px solid #C8E47A', color: '#4E7A1A', padding: '10px 14px', borderRadius: '10px', fontSize: '13px' }}>✓ Period: {Math.floor(yearDiff)} years selected.</div>
                )
              )}

              {form.latitude && form.longitude && (
                <div style={{ background: '#F4FAE8', border: '1px solid #C8E47A', borderRadius: '10px', padding: '10px 14px', fontSize: '13px', fontWeight: 600, color: '#4E7A1A', fontFamily: 'monospace' }}>📍 {form.latitude}, {form.longitude}</div>
              )}
              ...
              <button type="submit" disabled={loading || !isFormValid} style={{ width: '100%', padding: '13px', borderRadius: '12px', border: 'none', cursor: loading || !isFormValid ? 'not-allowed' : 'pointer', fontSize: '15px', fontWeight: 700, color: 'white', background: loading || !isFormValid ? '#C8E47A' : 'linear-gradient(135deg, #8DC63F, #6A9E28)', boxShadow: loading || !isFormValid ? 'none' : '0 4px 14px rgba(141,198,63,0.40)', marginTop: '4px' }}>
                {loading ? 'Submitting...' : form.job_type === 'download_only' ? 'Download Selected Data' : 'Generate TMY File'}
              </button>
            </form>
          </div>
        </div>
        ...
        {showDownloadOptions && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 99999 }}>
            <div style={{ background: 'white', padding: '2rem', borderRadius: '16px', width: '440px' }}>
              <h2 style={{ fontSize: '18px', fontWeight: 700, marginBottom: '1rem', color: '#1f2937' }}>Select Files To Download</h2>
              {['ERA5', 'CAMS', 'Temperature', 'GHI', 'DNI', 'GTI', 'Wind Speed', 'Relative Humidity'].map(item => (
                <label key={item} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px', fontSize: '14px', color: '#374151' }}>
                  <input type="checkbox" value={item} onChange={(e) => { if (e.target.checked) { setSelectedFiles(prev => [...prev, item]); } else { setSelectedFiles(prev => prev.filter(x => x !== item)); } }} />
                  {item}
                </label>
              ))}
              <button onClick={() => { setForm(prev => ({ ...prev, job_type: 'download_only' })); setShowDownloadOptions(false); }} style={{ marginTop: '1rem', background: '#8DC63F', color: 'white', border: 'none', padding: '10px 24px', borderRadius: '10px', fontSize: '14px', fontWeight: 600, cursor: 'pointer' }}>Confirm</button>
            </div>
          </div>
        )}
      </div>
