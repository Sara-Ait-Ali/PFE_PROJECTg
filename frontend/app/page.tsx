...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 font-sans">
      ...
      {/*
        ── BACKGROUND SETUP ──
        Layer 1 (bottom): /public/hero-bg.jpg  → the GEP building photo
        Layer 2 (middle): dark green multiply overlay → deepens the photo & unifies with brand
        Layer 3 (top):    radial green light blobs → the original brand reflets, now feel like
                          sunlight pouring through the building facade
        Result: real architecture shows through a signature green atmosphere.
      */}
      <section
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            backgroundPosition: 'center 30%',   /* shows the building facade, not the sky */
            backgroundRepeat: 'no-repeat',
          }}
        />

        {/* ── Layer 2: Deep green color-wash — ties photo to brand palette ── */}
        <div
          className="absolute inset-0"
          style={{
            background: 'linear-gradient(135deg, rgba(30,60,10,0.82) 0%, rgba(58,107,26,0.72) 50%, rgba(45,90,20,0.80) 100%)',
          }}
        />

        {/* ── Layer 3: Radial light blobs (the "green reflet") ── */}
        <div
          className="absolute inset-0 opacity-30"
          style={{
            backgroundImage: `
              radial-gradient(circle at 15% 50%, ${GRN} 0%, transparent 55%),
              radial-gradient(circle at 85% 20%, #8bc34a 0%, transparent 45%)
            `,
          }}
        />

        {/* ── Layer 4: Bottom fade so content below reads cleanly ── */}
        <div
          className="absolute bottom-0 left-0 right-0 h-32"
          style={{
            background: 'linear-gradient(to bottom, transparent, rgba(20,50,8,0.4))',
          }}
        />
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1">First name</label>
                  <input className="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2" placeholder="First name" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1">Last name</label>
                  <input className="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2" placeholder="Last name" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1">Email</label>
      ...
      <footer style={{ background: '#1e1e14' }} className="text-slate-300">
          ...
          <div className="max-w-7xl mx-auto px-6 py-8">
            ...
            <div className="flex flex-wrap justify-center items-center gap-10">
              {[
                ...
                <div key={sp.name} className="flex flex-col items-center gap-2 opacity-60 hover:opacity-100 transition">
                  <div className="h-20 w-48 rounded-lg flex items-center justify-center overflow-hidden px-4"
                    style={{ background: 'rgba(255,255,255,0.06)' }}>
                    <img src={sp.src} alt={sp.name}
        ...
        <div className="max-w-7xl mx-auto px-6 py-14 grid md:grid-cols-4 gap-10">
          ...
          <div>
            ...
            <p className="text-sm leading-relaxed" style={{ color: '#6a8a6a' }}>
              ClimaSphere is developed at the{' '}
              <span style={{ color: '#6a8a6a', fontWeight: 'bold'}}>Green Energy Park</span> — Africa's first renewable energy R&D platform in Ben Guerir, Morocco. A joint initiative by IRESEN and UM6P.
            </p>
          </div>
