...
//         {/* Page title */}
//         <div style={{ marginBottom: '24px' }}>
//           <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: '#EAF3DE', color: G, fontSize: '12px', fontWeight: 600, padding: '3px 10px', borderRadius: '999px', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
//             Job #{jobId}
//           </div>
//           <h1 style={{ fontSize: '26px', fontWeight: 800, color: '#14532d', margin: '0 0 4px' }}>Processing Status</h1>
//           <p style={{ color: '#6b7280', fontSize: '14px', margin: 0 }}>This page updates automatically every 5 seconds.</p>
...
async function downloadZip(jobId, siteName) {
  const token = localStorage.getItem('access_token') || '';
  const response = await fetch(`${BASE}/api/tmy/download/${jobId}/`, {
  ...
  const link = document.createElement('a');
  link.href  = url;
  link.setAttribute('download', `TMY_${siteName}.zip`);
  document.body.appendChild(link);
  link.click();
  link.remove();
  window.URL.revokeObjectURL(url);
                ...
                <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      setDlLoading(true);
                      try {
                        await downloadZip(jobId, job.site_name);
                      } catch {
                        alert('Download failed. Please try again.');
                      } finally {
                        setDlLoading(false);
                    ...
                    style={{ background: 'white', color: GD, border: `2px solid ${G}`, padding: '13px 32px', borderRadius: '12px', fontSize: '15px', fontWeight: 700, cursor: dlLoading ? 'not-allowed' : 'pointer', opacity: dlLoading ? 0.7 : 1, display: 'flex', alignItems: 'center', gap: '8px' }}
                    onMouseEnter={e => { if (!dlLoading) e.currentTarget.style.background = GL; }}
                    onMouseLeave={e => { if (!dlLoading) e.currentTarget.style.background = 'white'; }}
                  >
                    {dlLoading ? 'Preparing…' : '⬇ Download Results'}
                  </button>
